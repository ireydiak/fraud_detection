...
class MLPTrainer:
    ...
    def test(self, dataset):
        ...
        with torch.no_grad():
            for row in dataset:
                X, y = row[:, :row.shape[1] - 1], row[:, row.shape[1] - 1]
        ...
        for epoch in range(num_epochs):
            ...
            with tqdm(range(len(train_loader))) as t:
                ...
                for i, row in enumerate(train_loader, 0):
                    ...
                    train_inputs = row[:, :row.shape[1] - 1].to(self.device, dtype=torch.float).float()
                    train_labels = row[:, row.shape[1] - 1].to(self.device, dtype=torch.long).float()
...
class InstaCartDataset(Dataset):
    ...
    def loaders(self, num_workers: int = 4) -> Tuple[DataLoader, DataLoader]:
        train_set, train_set = self.train_test_split(self.test_ratio)
        train_ldr = DataLoader(dataset=train_set, batch_size=self.batch_size, num_workers=num_workers)
        test_ldr = DataLoader(dataset=train_set, batch_size=self.batch_size, num_workers=num_workers)
...
def main():
    ...
    data = df.to_numpy()
    cls_0_ratio = (data[:, -1] == 0).sum() / len(data)
    cls_1_ratio = (data[:, -1] == 1).sum() / len(data)
    class_ratio = {0: cls_0_ratio, 1: cls_1_ratio}
    dataset = InstaCartDataset(data, class_ratio=class_ratio)
    ...
    print(metrics)
    results.append([metrics["Precision"], metrics["Recall"], metrics["F1-Score"]])
    summary_df = pd.DataFrame(
        results,
        columns=metrics.keys(),
        index=[x.name for x in [MLP5, MLP3, MLP2]]
    )
